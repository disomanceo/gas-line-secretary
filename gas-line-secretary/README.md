...
- ก่อนครบกำหนด 5 นาที ระบบจะแจ้งเตือนรายชื่อครูที่ยังไม่รับทราบ
- เมื่อครบกำหนด ระบบจะส่งสรุปแบบ Flex Message
- ถ้ากดรับทราบหลังครบกำหนด ระบบจะบันทึกเป็น `late`
...
ระบบจะเพิ่มคอลัมน์ใหม่ให้อัตโนมัติโดยไม่ล้างข้อมูลเดิม เช่น:

```text
Announcements: reminderSentAt, summarySentAt
Acknowledgements: ackStatus
```
...
ระบบจะเช็กทุก 5 นาที เพื่อส่งแจ้งเตือนก่อนครบกำหนดประมาณ 5 นาที และสรุปคนที่ยังไม่รับทราบเมื่อครบกำหนด

ถ้าเคยติดตั้ง trigger เดิมไว้แล้ว ให้รัน `installDeadlineTrigger()` ใหม่หนึ่งครั้ง ระบบจะลบ trigger เดิมของ `checkDeadlines` แล้วสร้างตัวใหม่ให้เอง
